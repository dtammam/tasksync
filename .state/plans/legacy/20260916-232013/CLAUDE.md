...
Completed plans:
- `docs/exec-plans/completed/2026-07-19-distinguishable-task-api-errors.md` — Distinguishable error responses for `POST /api/tasks`: module-private `ApiTaskError` two-shape enum (`Concealed` = byte-for-byte `Err(StatusCode::NOT_FOUND)` empty body / no `Content-Type`, reserved for the feature-off gate; `Coded { status, code }` = `(status, Json(ErrorBody { error: { code, message } }))`) with per-call-site `from_auth`/`from_create` mappers + `forbidden_scope()` constructor + static `message_for_code` lookup, translating each post-gate failure to a stable `snake_case` code (`unauthorized` 401, `forbidden_scope` 403, `unknown_list` 404, `invalid_request` 400, `conflict` 409, `forbidden` 403, `internal_error` 500) so an automation caller can tell the two 404s apart by body alone; boundary-only change in `server/src/routes/integrations.rs` leaving shared `create_task_for_ctx` and the browser `POST /tasks` bare-status-code contract untouched; per-category `tracing` rejection logs (client-caused `warn` / server-fault `error`) with a pure testable `reject_log_message` helper and the raw `X-TaskSync-Api-Token` never passed to any log macro; feature-off 404 concealment kept byte-for-byte identical (no log line, no body); accepted 12/12 criteria (complete 2026-07-19)
- `docs/exec-plans/completed/2026-07-18-gated-login-wall-and-task-api.md` — Gated login wall + programmatic task API: full-screen `LoginWall` client gate (first-run owner setup via `GET /auth/status` + `POST /auth/setup`, no app content painted while anonymous, Sidebar login form removed), per-user `token_version` revocation folded into the existing `ctx_from_headers` identity lookup (JWT `tv` claim, `serde(default)` legacy-safe) with `POST /auth/revoke-sessions` + password-change bumps, optional `TASK_API_TOKEN` create-task-only ingest route `POST /api/tasks` (constant-time compare, owner-scoped, one-route wiring, 404 when disabled), and #047 closure (hash-only auth, `DEV_LOGIN_PASSWORD` shared fallback removed, boot preflight keeps `JWT_SECRET` mandate); accepted 19/19 criteria (complete 2026-07-18)
- `docs/exec-plans/completed/2026-07-01-auth-hardening.md` — Auth hardening: header-auth bypass removal (Bearer-only `ctx_from_headers`, sync passes `RequestCtx` directly, real-JWT test migration), fail-closed boot preflight on unset/default `JWT_SECRET`/`DEV_LOGIN_PASSWORD` (+dotenvy local dev), additive `CORS_ALLOWED_ORIGINS` allow-list, ARCHITECTURE.md sync-doc reconciliation; branch 1 of 3 in the remediation arc; #047 `DEV_LOGIN_PASSWORD` shared-fallback-login flagged High for branch 2 (complete 2026-07-02)
...
- `docs/exec-plans/completed/2026-05-07-fix-task-list-hidden-behind-input.md` — Fix task list items hidden behind fixed add-task input: added `.page-content` wrapper with bottom padding to list route, CSS-only fix (complete 2026-05-07)
