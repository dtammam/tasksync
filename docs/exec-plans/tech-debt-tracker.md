...
| 052 | Test/CI | Low | `tests/e2e/task-tags.spec.ts` sets a task emoji by writing IndexedDB directly (`updateTaskInIdb`) then `page.reload()`, which can race the app's periodic `repo.saveTasks()` (clears + re-puts the in-memory, emoji-less tasks) and clobber the edit → wrong grouping. Also uses weak `toHaveCount(0)` assertions (e.g. `:129`) satisfied before content renders. SUSPECTED (not CI-confirmed); surfaced during the fix/e2e-flake-elimination sweep and deliberately deferred to avoid destabilizing a currently-passing test | unassigned | Set the tag through the emoji-picker UI (so the app owns the write and in-memory + IDB stay consistent) rather than a direct IDB write; anchor each `toHaveCount(0)` on a positive assertion first |

## Closed

| ID | Area | Closed on | Summary | Link |
|---:|------|-----------|---------|------|
| 051 | Test/CI | 2026-09-20 | webkit `pull-to-refresh.spec.ts` wheel-gesture test raced the component's real 150ms end-detection debounce via Playwright locator round-trips. Fixed by dispatching the WheelEvents AND reading opacity/transform inside one `page.evaluate()` with a microtask-bounded in-page read (no automation round trip in the timed window). Adversary-verified by mutation A/B; CI green on webkit | [#154](https://github.com/dtammam/tasksync/pull/154) |
| 050 | Test/CI | 2026-09-20 | `offline.spec.ts` readiness flakes (both signatures) — root cause: `data-ready` reflected only local/seed hydration, not server-sync-applied state, and e2e ran against the Vite dev server whose SW precaches nothing (offline reload couldn't boot). Fixed by a `data-synced` marker + `expectAppSynced` helper, cache-first auth boot, and running e2e against a production build (`vite preview`); firefox skips the offline describe (Playwright SW+offline limitation), chromium is the offline-coverage browser. CI green chromium/firefox/webkit | [#154](https://github.com/dtammam/tasksync/pull/154) |
| 053 | Test/CI | 2026-09-20 | `sidebar-drag.spec.ts` intermittently saw 5 sidebar items (the built-in seed lists) instead of the 2 mocked lists — the startup `/sync/pull` hadn't replaced the seeds yet and `data-synced` can settle on an errored/empty first pull. Fixed by waiting for a mocked list by identity (30s bound) then asserting exactly the two + no seed remains. Verified 30/30 firefox+chromium under load; CI green. **Follow-up (PR #159):** the 30s identity wait still flaked on **firefox** CI (mocked `/sync/pull` not applied within 30s — the same firefox+Playwright mock-sync limitation that makes firefox skip the offline describe), so `sidebar-drag` now skips on firefox; chromium + webkit cover it | [#154](https://github.com/dtammam/tasksync/pull/154), [#159](https://github.com/dtammam/tasksync/pull/159) |
