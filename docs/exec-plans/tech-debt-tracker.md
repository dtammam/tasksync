...
| 045 | Server | Low | Drop legacy `user.streak_state_json` column. Retained during the server-authoritative streak rollout (`docs/exec-plans/active/2026-05-10-server-authoritative-streak.md`) for rolling-deploy compatibility — newer server reads canonical state from `user_streak` and silently ignores PATCH writes to the legacy field, but the column itself is still in the schema. User has approved dropping it in a follow-up migration ~1 stable release week after the server-authoritative streak ships and is verified in production. Do not drop earlier — older clients still reading `GET /auth/preferences` need the legacy field populated by migration `0017_user_streak.sql` until they are off the network. | unassigned | After server-authoritative streak has been live in production for one full release week with no rollback, author migration `0018_drop_user_streak_state_json.sql` to drop the column and remove the seed/read paths in `server/src/routes/auth.rs`. |
| 046 | Store | Low | `streak.break()` always sends `cause: 'manual'` to the server regardless of the caller's intent (3 callers in `web/src/lib/stores/streak.ts` at the daily-reset and missed-task sites, plus task-side callers). The server `break` op-kind contract accepts `'punt' | 'skip' | 'delete' | 'manual'` for analytics/debugging telemetry, but the client currently collapses all four to `'manual'`. Behavior is correct (all four cause types reset the count to 0 identically), but the cause field is wasted as a debug signal. | unassigned | Refactor `streak.break()` to accept an optional `cause: StreakBreakCause` parameter, plumb it through `buildBreakOp`, and update the 3 callers in `streak.ts` plus any task-side callers to pass the appropriate cause (`'skip'` for skip-day, `'punt'` for punt, `'delete'` for delete-task, `'manual'` only for unknown/other). |
