...
| 050 | Test/CI | Medium | `tests/e2e/offline.spec.ts`'s reload-based tests (`offline title edit survives reload...`, `offline list edit survives reload...`, others in the same "Offline continuity" describe block) intermittently fail on GitHub Actions' chromium runner specifically — 3 failures in 4 consecutive runs on PR #141's final sha, each a different test in the block, always the same signature: `getByTestId('app-shell')` doesn't reach `data-ready="true"` within the 10000ms timeout after `page.reload({waitUntil:'domcontentloaded'})` while the page is offline. Passes 100% locally (8+ isolated re-runs, 2-worker full-suite runs) and 100% on firefox/webkit in the same CI runs — points to the 10s timeout being marginal specifically for the offline+service-worker+reload boot path under GitHub's shared chromium runner load, not a logic bug (confirmed unrelated to `feat/task-emoji`'s diff, which touches no service-worker/offline/boot code) | unassigned | Either raise the timeout for this describe block specifically (e.g. 20s) or investigate why offline-mode reload boot is slow enough on CI's chromium runner to be marginal against a 10s budget that mobile/normal-boot tests never hit |
