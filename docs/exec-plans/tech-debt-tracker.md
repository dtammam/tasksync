...
| 050 | Test/CI | Medium | Multiple E2E tests with tight, timing-sensitive assertions are marginal specifically on GitHub Actions' shared runners, not locally: (a) `tests/e2e/offline.spec.ts`'s reload-based tests (`offline title edit/list edit survives reload...`) on **chromium** — 3 failures in 4 consecutive runs on PR #141's final sha, always `getByTestId('app-shell')` missing `data-ready="true"` within a 10000ms timeout after an offline reload; (b) `tests/e2e/pull-to-refresh.spec.ts:181` ("PTR wheel gesture...") on **webkit** — 2 failures in 3 consecutive runs on PR #140, always a CSS/`style` attribute (`translateY(...)`/`opacity`) not settling within a 10000ms timeout. Both pass 100% locally and 100% on the *other* browsers in the same CI runs, and neither PR's diff touched service-worker/offline/PTR-animation code — confirms marginal timeout budgets under CI runner load, not logic bugs, and not confined to one browser or one test file. **Owner direction 2026-09-18: pause other roadmap work for a live pairing session on this once the current piece ships** | unassigned | Live pairing session with the owner: investigate CI runner performance/config directly rather than just raising timeouts blind; if a root cause isn't found, raise timeouts for animation/reload-dependent assertions specifically (e.g. 20s) as a fallback |
