...
- [x] `isRecurrenceRule('lastDayOfMonth')` returns `true`.
- [x] `isRecurrenceRule('monthly')` still returns `true` (no regression to existing rules).
- [x] `nextDueForRecurrence` with `'lastDayOfMonth'` returns the last calendar day of the following month for every month of the year:
...
- [x] `prevDueForRecurrence` with `'lastDayOfMonth'` returns the last calendar day of the preceding month:
  - Feb 28 (non-leap 2026) -> Jan 31
  - Mar 31 -> Feb 28 (non-leap) or Feb 29 (leap)
  - Jan 31 -> Dec 31 of the prior year
- [x] The task-detail recurrence dropdown includes a `'Last day of month'` option (visible in the UI).
- [x] Selecting `'lastDayOfMonth'` in the dropdown persists the value to the task's `recur_state` field (verified by E2E `@smoke` test).
- [x] Completing a `lastDayOfMonth` recurring task rolls the due date to the last day of the next month (verified via unit test on `nextDueForRecurrence`).
- [x] All 8 existing recurrence rules pass their existing unit tests without modification (no regression).
- [x] All quality gates pass: `npm run lint`, `npm run check`, `npm run test`, Playwright `@smoke` (Chromium).
...
- **`web/tests/e2e/` (E2E spec)**: One `@smoke`-tagged Playwright test that opens a task detail, selects "Last day of month" from the recurrence dropdown, and verifies persistence.
...
- 2026-04-01: All 4 tasks implemented — rule registration, helper + switch cases, 28 unit tests (13 forward + 14 backward + 1 isRecurrenceRule), E2E smoke test.
- 2026-04-01: Acceptance complete — all 9 acceptance criteria pass. 313 unit tests + 14 E2E smoke tests green. All quality gates clean.
- 2026-04-01: Feature complete. Exec plan moved to completed.
