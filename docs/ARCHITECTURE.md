...
- **Streak (combo meter):** Optional DDR-style task-completion streak. Increments on completion, breaks on punt/delete. Supports DDR (daily reset) and Endless modes. Settings persisted cross-device via `/auth/preferences` as `streak_settings_json`. Canonical streak state (count, lastResetDate, dayCompleteDate, revision) is server-authoritative — clients send idempotent operations (`increment`, `break`, `day_complete`, `reset`) to `POST /auth/streak/op`, which applies them transactionally and returns the canonical record. The endpoint deduplicates on a stable `opKey` per operation, so retries after a network blip do not double-count. Cross-device propagation rides the existing `GET /auth/preferences` poll (5-minute interval + visibility-restore + post-drain) which embeds the canonical `streakStateJson` derived from `user_streak` plus a monotonic `streakRevision` cursor; clients silently reconcile in place when the server's revision exceeds the locally-seen one (no animation, no sound). Theme assets (digit PNGs, streak-word PNG, judgment PNGs, announcer MP3s) declared in a per-theme `manifest.json` under `web/static/streak/{theme}/`. Announcer fires at count 5 and then probabilistically every 10–20 completions; at milestones the announcer replaces the regular completion sound for that event. Day-complete celebration fires once per calendar day when all My Day tasks are done; first-to-server wins, and racing devices that lose silently suppress the celebration. Optimistic local update fires synchronously on completion to preserve the <20 ms sound-onset budget; the outbound op rides an IDB-backed queue (`streakOps` store) that drains on network availability and replays idempotently.
...
### Server-authoritative streak (migration `0017_user_streak.sql`)

- **`user.streak_state_json` is read-only after this deploy.** The server derives canonical `streakStateJson` from the `user_streak` table on `GET /auth/preferences`. Migration `0017_user_streak.sql` seeds `user_streak` from the legacy column for backwards compatibility, but the column is no longer the authoritative source on the read path.
- **Older clients' `PATCH /auth/preferences` writes that include `streakStateJson` are silently dropped server-side.** The drop prevents stale blob-pushing clients from clobbering canonical state. The server logs the discard at `tracing::debug!` level and returns a normal success response — the client sees no error.
- **The legacy column will be removed in a follow-up migration (`0018_drop_user_streak_state_json.sql`) approximately one stable release week after the server-authoritative streak ships with no rollback.** This is tracked as tech-debt entry **#045** in `docs/exec-plans/tech-debt-tracker.md`. Do not author or run `0018` until the feature has been live in production for a full release week, because older clients still polling `GET /auth/preferences` rely on the column being populated by `0017`.
